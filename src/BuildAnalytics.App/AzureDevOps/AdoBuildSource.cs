...
/// <summary>
/// Azure DevOps build source: one paged build-list GET per call (ADR-55). Holds the
/// per-session run budget; the pipeline owns paging, repeated-token detection, and restart.
/// </summary>
public sealed class AdoBuildSource : IBuildSource, IDisposable
{
    ...
    public async Task<BuildPage> ListAsync(BuildQuery query, string? continuationToken, CancellationToken cancellationToken)
    {
        ...
        var response = await _executor
            .SendAsync(() => CreateRequest(uri), path, continuationToken, cancellationToken)
        ...
        {
            var run = AdoJsonMapper.MapBuild(item, fetchedAt);
            if (run.Id > 0)
            {
                runs.Add(run);
            }
