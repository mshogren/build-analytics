...
public sealed class AdoRequestExecutorTests
{
    ...
    public async Task Empty_continuation_header_is_treated_as_no_token()
    {
        ...
        var response = await executor.SendAsync(Factory, "/project/_apis/build/builds", null, default);
    ...
    public async Task Retryable_status_is_retried(HttpStatusCode status)
    {
        ...
        await executor.SendAsync(Factory, "/project/_apis/build/builds", null, default);
    ...
    public async Task Correlation_id_priority_request_id_activity_then_correlation()
    {
        ...
        var exception2 = await Assert.ThrowsAsync<AdoRequestException>(
            () => executor2.SendAsync(Factory, "/project/_apis/build/builds", null, default));
    ...
    public async Task BadRequest_with_continuation_token_is_InvalidContinuationTokenException()
    {
        ...
        var exception = await Assert.ThrowsAsync<InvalidContinuationTokenException>(
            () => executor.SendAsync(Factory, "/project/_apis/build/builds", "stale", default));
    ...
    public async Task Cancelled_caller_token_is_rethrown_without_retry()
    {
        ...
        await Assert.ThrowsAsync<OperationCanceledException>(
            () => executor.SendAsync(Factory, "/project/_apis/build/builds", null, cts.Token));
